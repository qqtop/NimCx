...
##                   
## 
##                   cxprint2.nim has backgroundcolor set to bgDefault
## 
...
proc print*[T](astring:T,fgr:string = termwhite ,bgr:BackgroundColor = bgDefault,xpos:int = 0,fitLine:bool = false ,centered:bool = false,styled : set[Style]= {},substr:string = "")
proc printLn*[T](astring:T,fgr:string = termwhite , bgr:BackgroundColor = bgDefault,xpos:int = 0,fitLine:bool = false,centered:bool = false,styled : set[Style]= {},substr:string = "")
...
proc print*[T](astring :T,
    ...
    {.gcsafe.}:
        ...
        if fgr != $fgWhite or bgr != bgDefault:
...
proc rainbow*[T](s : T,
    ...
    for x in 0..<astr.len:
       ...
       if centered == false:
          print(astr[x],colorNames[c][1],bgDefault,xpos = nxpos,fitLine)
       else:
...
proc printBiCol*[T](s:varargs[T,`$`],
     ...
     {.gcsafe.}:
        ...
        if nosepflag == false:
                ...
                if centered == false:
                    print(z[0],fgr = colLeft,bgr = bgDefault,xpos = xpos,styled = styled)
                    print(z[1],fgr = colRight,bgr = bgDefault,styled = styled)
                else:  # centered == true
                    let npos = centerX() - (zz).len div 2 - 1
                    print(z[0],fgr = colLeft,bgr = bgDefault,xpos = npos,styled = styled)
                    print(z[1],fgr = colRight,bgr = bgDefault,styled = styled)
        ...
        if nosepflag == false:
            ...
            if centered == false:
                ...
                if colRight == clrainbow:   # we currently do this as rainbow implementation has changed
                        printLn(z[1],fgr = randcol(),bgr = bgDefault,styled = styled)
                else:
                        printLn(z[1],fgr = colRight,bgr = bgDefault,styled = styled)

            else:  # centered == true
                let npos = centerX() - zz.len div 2 - 1
                print(z[0],fgr = colLeft,bgr = bgDefault,xpos = npos)
